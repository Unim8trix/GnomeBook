...
install() {
    ...
    echo -e "${YELLOW}Install some requirements${NORMAL}\n"
    sleep 2
    su ${USERNAME} -c "yay --noconfirm -Sy vulkan-radeon mesa \
      noto-fonts-emoji noto-fonts pipewire-alsa pipewire-pulse pipewire-jack"
    echo -e "${GREEN}Done${NORMAL}\n"
    sleep 5
    ...
    echo -e "${YELLOW}Enable Gnome display manager${NORMAL}\n"
    systemctl enable gdm
    echo -e "${GREEN}Done${NORMAL}\n"
    sleep 1

    echo -e "${YELLOW}Adjust Gnome settings${NORMAL}\n"
    su ${USERNAME} -c "gsettings set org.gnome.SessionManager logout-prompt false"
    echo -e "${GREEN}Done${NORMAL}\n"
    sleep 1
    ...
    su ${USERNAME} -c "mkdir -p /home/${USERNAME}/{.cache/nano/backups,.local/share/fonts,Bilder/Wallpaper}"
    echo -e "${GREEN}Done${NORMAL}\n"
    sleep 1
}
